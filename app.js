...
const CALL_OBJECTIVE_REVERSE_MAP = {
  QUALIFY: "Qualify",
  PITCH: "Pitch",
  CLOSE: "Close",
...
const CALL_ATTEMPT_REVERSE_MAP = {
  OPT1ST_ATTEMPT: "1st Attempt",
  OPT2ND_ATTEMPT: "2nd Attempt",
  FOLLOW_UP: "Follow-up",
  CLOSING_CALL: "Closing Call"
};

const CONFIDENCE_REVERSE_MAP = {
  HIGH: "High",
  MEDIUM: "Medium",
  LOW: "Low"
};

const FIT_DECISION_REVERSE_MAP = {
  STRONG_FIT: "Strong fit",
  MODERATE_FIT: "Moderate fit",
  NOT_FIT: "No fit"
};

const CLOSE_ATTEMPTED_REVERSE_MAP = {
  ASKED_TO_ENROLL: "Asked to enroll",
  SCHEDULED_CALL: "Scheduled call",
  NO_CLOSE: "No close"
};

const DECISION_AT_PITCH_REVERSE_MAP = {
  ENROLLED: "Enrolled",
  WANTS_TIME: "Wants time",
  OBJECTIVE: "Objected"
};

const PRIMARY_OBJECTION_REVERSE_MAP = {
  PRICE: "Price",
  TIME: "Time",
  AUTHORITY: "Authority",
  FIT: "Fit",
...
const FINAL_OUTCOME_REVERSE_MAP = {
  WON_ENROLLED: "Won / enrolled",
  FOLLOW_UP_SCHEDULED: "Follow-up scheduled",
  LOST_OBJECTION_UNHANDLED: "Lost - objection unhandled",
  LOST_TIMING: "Lost - timing",
  DISQUALIFIED: "Disqualified"
};

const NEXT_ACTION_REVERSE_MAP = {
  SEND_PAYMENT_LINK: "Send payment link",
  SEND_CURRICULUM: "Send curriculum",
  SEND_PROOF_CASE_STUDY: "Send proof/case study",
  SCHEDULE_FOLLOW_UP: "Schedule follow-up",
  SEND_RECORDING: "Send recording",
  CLOSE_LEAD: "Close lead"
};

const READY_NEXT_CALL_REVERSE_MAP = {
  YES: "Yes",
  NO_REVIEW_NEEDED: "No - review needed"
};

function parseTiming(timingStr) {
  if (!timingStr) return { totalFormatted: "00:00", prepSec: 0, liveSec: 0, wrapSec: 0, totalSec: 0 };
  const str = String(timingStr).trim();
  const m = str.match(/^([0-9:]+)(?:\s*\(Prep:\s*([0-9:]+),\s*Live:\s*([0-9:]+),\s*Wrap:\s*([0-9:]+)\))?/i);
  const parseSec = (t) => {
    if (!t) return 0;
  ...
  if (m) {
    const totalFormatted = m[1] || "00:00";
    const prepSec = parseSec(m[2]);
    const liveSec = parseSec(m[3]);
    const wrapSec = parseSec(m[4]);
    const sumSec = prepSec + liveSec + wrapSec;
    return {
      totalFormatted,
      prepSec,
      liveSec,
      wrapSec,
      totalSec: sumSec > 0 ? sumSec : parseSec(totalFormatted)
    };
  }
  return { totalFormatted: str, prepSec: 0, liveSec: 0, wrapSec: 0, totalSec: parseSec(str) };
}
...
function getFieldText(field) {
  ...
  if (typeof field === "object") {
    ...
    if (field.blocknote) {
      try {
        const parsed = JSON.parse(field.blocknote);
        if (Array.isArray(parsed)) {
          return parsed.map(p => (p.content || []).map(c => c.text || "").join("")).filter(Boolean).join("\n");
        }
      } catch (e) { }
    }
  }
...
function parseTargetDimension(rawField, prefixes = []) {
  const str = getFieldText(rawField).trim();
  if (!str) return { know: "", why: "" };

  const whyRegex = /\n+(?:Why(?:\s+this\s+matters)?|Why):\s*([\s\S]*)$/i;
  const whyMatch = str.match(whyRegex);
  let why = "";
  let know = str;
  if (whyMatch) {
    why = whyMatch[1].trim();
    know = str.slice(0, whyMatch.index).trim();
  }

  for (const prefix of prefixes) {
    const pRegex = new RegExp("^" + prefix + ":?\\s*", "i");
    if (pRegex.test(know)) {
      know = know.replace(pRegex, "").trim();
      break;
    }
  }
  return { know, why };
...
function parseProbes(rawField) {
  ...
  const str = getFieldText(rawField).trim();
  if (!str) return defaultProbes;

  const probeRegex = /Probe\s*(\d+)[:\.]?\s*([\s\S]*?)(?=(?:Probe\s*\d+[:\.]?|$))/gi;
  let matches = [...str.matchAll(probeRegex)];
  if (matches.length > 0) {
    const parsedList = [];
    matches.forEach(m => {
      ...
      if (idx >= 0) {
        const body = m[2].trim();
        let q = "", motive = "", followup = "";
        if (body.includes("|")) {
          const parts = body.split(/\s*\|\s*/);
          q = parts[0] || "";
          for (let i = 1; i < parts.length; i++) {
            const p = parts[i].trim();
            if (/^Motive(?:\s*Probed)?:\s*/i.test(p)) {
              motive = p.replace(/^Motive(?:\s*Probed)?:\s*/i, "").trim();
            } else if (/^Follow-up:\s*/i.test(p)) {
              followup = p.replace(/^Follow-up:\s*/i, "").trim();
            }
          }
        } else {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            if (/^-\s*Motive(?:\s*Probed)?:\s*/i.test(l)) {
              motive = l.replace(/^-\s*Motive(?:\s*Probed)?:\s*/i, "").trim();
            } else if (/^-\s*Follow-up:\s*/i.test(l)) {
              followup = l.replace(/^-\s*Follow-up:\s*/i, "").trim();
            }
          }
        }
        parsedList[idx] = { q: q.trim(), m: motive, f: followup };
      }
    });
    const cleaned = [];
    for (let i = 0; i < parsedList.length; i++) {
      if (parsedList[i]) cleaned.push(parsedList[i]);
    }
    return cleaned.length > 0 ? cleaned : defaultProbes;
  }
  defaultProbes[0].q = str;
  return defaultProbes;
}

function parseQuestions(rawField) {
  const str = getFieldText(rawField).trim();
  if (!str) return { q1: "", q2: "", q3: "" };
  const m1 = str.match(/Q1:\s*([\s\S]*?)(?=(?:Q2:|$))/i);
  const m2 = str.match(/Q2:\s*([\s\S]*?)(?=(?:Q3:|$))/i);
  const m3 = str.match(/Q3:\s*([\s\S]*?)$/i);
  if (m1 || m2 || m3) {
    return {
      q1: m1 ? m1[1].trim() : "",
      q2: m2 ? m2[1].trim() : "",
...
function parseResponses(rawField) {
  const str = getFieldText(rawField).trim();
  if (!str) return { ans1: "", ans2: "", ans3: "" };
  const m1 = str.match(/(?:Ans\s*1|A1):\s*([\s\S]*?)(?=(?:(?:Ans\s*2|A2):|$))/i);
  const m2 = str.match(/(?:Ans\s*2|A2):\s*([\s\S]*?)(?=(?:(?:Ans\s*3|A3):|$))/i);
  const m3 = str.match(/(?:Ans\s*3|A3):\s*([\s\S]*?)$/i);
  ...
  return { ans1: str, ans2: "", ans3: "" };
}

function parseFollowUps(rawField) {
  const str = getFieldText(rawField).trim();
  if (!str) return { f1: "", f2: "", f3: "" };
  const m1 = str.match(/(?:Follow-up\s*1|F1):\s*([\s\S]*?)(?=(?:(?:Follow-up\s*2|F2):|$))/i);
  const m2 = str.match(/(?:Follow-up\s*2|F2):\s*([\s\S]*?)(?=(?:(?:Follow-up\s*3|F3):|$))/i);
  const m3 = str.match(/(?:Follow-up\s*3|F3):\s*([\s\S]*?)$/i);
  if (m1 || m2 || m3) {
    return {
      f1: m1 ? m1[1].trim() : "",
      f2: m2 ? m2[1].trim() : "",
...
function parseSignals(rawField) {
  const str = getFieldText(rawField).trim();
  let sig1 = [], sig2 = [];
  if (!str) return { sig1, sig2 };
  const m1 = str.match(/Q1\s*Signals:\s*([^|]+)/i);
  if (m1) {
    sig1 = m1[1].split(",").map(s => s.trim()).filter(Boolean);
  }
  const m2 = str.match(/Q2\s*Signals:\s*([^|]+)/i);
  if (m2) {
    sig2 = m2[1].split(",").map(s => s.trim()).filter(Boolean);
  }
  return { sig1, sig2 };
}

function parseDiscovery(rawQ, rawAns, rawSigs, rawFs) {
  const qStr = getFieldText(rawQ).trim();
  const ansStr = getFieldText(rawAns).trim();
  const sigStr = getFieldText(rawSigs).trim();
  const fStr = getFieldText(rawFs).trim();

  const qRegex = /Q\s*(\d+)[:\.]?\s*([\s\S]*?)(?=(?:Q\s*\d+[:\.]?|$))/gi;
  const qMap = {};
  let qm;
  while ((qm = qRegex.exec(qStr)) !== null) {
    qMap[parseInt(qm[1], 10)] = qm[2].trim();
  }

  const ansRegex = /(?:Ans\s*|A)(\d+)[:\.]?\s*([\s\S]*?)(?=(?:(?:Ans\s*|A)\d+[:\.]?|$))/gi;
  const ansMap = {};
  let am;
  while ((am = ansRegex.exec(ansStr)) !== null) {
    ansMap[parseInt(am[1], 10)] = am[2].replace(/^["']|["']$/g, "").trim();
  }

  const fRegex = /(?:Follow-up\s*|F)(\d+)[:\.]?\s*([\s\S]*?)(?=(?:(?:Follow-up\s*|F)\d+[:\.]?|$))/gi;
  const fMap = {};
  let fm;
  while ((fm = fRegex.exec(fStr)) !== null) {
    fMap[parseInt(fm[1], 10)] = fm[2].trim();
  }

  const sigRegex = /Q\s*(\d+)\s*Signals?:\s*([^|\n]+)/gi;
  const sigMap = {};
  let sm;
  while ((sm = sigRegex.exec(sigStr)) !== null) {
    sigMap[parseInt(sm[1], 10)] = sm[2].split(",").map(s => s.trim()).filter(Boolean);
  }

  const maxIndex = Math.max(
    3,
    ...Object.keys(qMap).map(Number),
    ...Object.keys(ansMap).map(Number),
    ...Object.keys(fMap).map(Number),
    ...Object.keys(sigMap).map(Number)
  );

  const list = [];
  for (let i = 1; i <= maxIndex; i++) {
    const qVal = qMap[i] || (i === 1 && !Object.keys(qMap).length ? qStr : "");
    const ansVal = ansMap[i] || (i === 1 && !Object.keys(ansMap).length ? ansStr : "");
    const fVal = fMap[i] || (i === 1 && !Object.keys(fMap).length ? fStr : "");
    const sigVal = sigMap[i] || (i === 1 && !Object.keys(sigMap).length ? (sigStr ? [sigStr] : []) : []);
    list.push({
      q: qVal,
      ans: ansVal,
      sigs: sigVal,
...
function getDiscoveryList(data) {
  if (Array.isArray(data?.discovery) && data.discovery.length > 0) {
    return data.discovery;
  }
  return [
    { q: data?.discQ1 || "", ans: data?.discAns1 || "", sigs: Array.isArray(data?.sig1) ? data.sig1 : [], f: data?.discF1 || "" },
    { q: data?.discQ2 || "", ans: data?.discAns2 || "", sigs: Array.isArray(data?.sig2) ? data.sig2 : [], f: data?.discF2 || "" },
    { q: data?.discQ3 || "", ans: data?.discAns3 || "", sigs: [], f: data?.discF3 || "" }
  ];
}

function parseObjections(rawField) {
  const defaultObjs = [
    { exact: "", cause: "", resp: "", res: "Resolved" },
    { exact: "", cause: "", resp: "", res: "" }
  ];
  const str = getFieldText(rawField).trim();
  if (!str || str.toLowerCase().includes("no objections")) return defaultObjs;

  const objRegex = /(?:Obj|Objection)\s*(\d+)[:\.]?\s*([\s\S]*?)(?=(?:(?:Obj|Objection)\s*\d+[:\.]?|$))/gi;
  const matches = [...str.matchAll(objRegex)];
  if (matches.length > 0) {
    matches.forEach(m => {
      const idx = parseInt(m[1], 10) - 1;
      if (idx >= 0 && idx < 2) {
        ...
        if (body.includes("|")) {
          const parts = body.split(/\s*\|\s*/);
          let exact = (parts[0] || "").replace(/^["']|["']$/g, "").trim();
          let cause = "", resp = "", res = "Resolved";
          for (let i = 1; i < parts.length; i++) {
            const p = parts[i].trim();
            if (/^(?:Root|Root\s*Cause):\s*/i.test(p)) {
              cause = p.replace(/^(?:Root|Root\s*Cause):\s*/i, "").trim();
            } else if (/^Response:\s*/i.test(p)) {
              resp = p.replace(/^Response:\s*/i, "").trim();
            } else if (/^(?:Resolved|Resolution):\s*/i.test(p)) {
              res = p.replace(/^(?:Resolved|Resolution):\s*/i, "").trim();
            }
          }
          defaultObjs[idx] = { exact, cause, resp, res: res || "Resolved" };
        } else {
          const lines = body.split("\n");
          let exact = (lines[0] || "").replace(/^["']|["']$/g, "").trim();
          let cause = "", resp = "", res = "Resolved";
          for (let i = 1; i < lines.length; i++) {
            const l = lines[i].trim();
            if (/^-\s*(?:Root\s*Cause|Root):\s*/i.test(l)) {
              cause = l.replace(/^-\s*(?:Root\s*Cause|Root):\s*/i, "").trim();
            } else if (/^-\s*Response:\s*/i.test(l)) {
              resp = l.replace(/^-\s*Response:\s*/i, "").trim();
            } else if (/^-\s*(?:Resolution|Resolved):\s*/i.test(l)) {
              res = l.replace(/^-\s*(?:Resolution|Resolved):\s*/i, "").trim();
            }
          }
          defaultObjs[idx] = { exact, cause, resp, res: res || "Resolved" };
        }
  ...
  defaultObjs[0].exact = str;
  return defaultObjs;
}

function parseActionDetails(rawField) {
  let owner = "", due = "", actions = [], collateral = "";
  const str = getFieldText(rawField).trim();
  if (!str) return { owner, due, actions, collateral };
  const ownerMatch = str.match(/Owner:\s*([^|]+)/i);
  if (ownerMatch) owner = ownerMatch[1].trim();
  const dueMatch = str.match(/Due:\s*([^|]+)/i);
  if (dueMatch) due = dueMatch[1].trim();
  const actMatch = str.match(/Actions:\s*([^|]+)/i);
  if (actMatch) actions = actMatch[1].split(",").map(s => s.trim()).filter(Boolean);
  const colMatch = str.match(/Collateral:\s*([\s\S]*)$/i);
  if (colMatch) collateral = colMatch[1].trim();
  return { owner, due, actions, collateral };
}

function parseQualitySignOff(rawField) {
  let score = "5", checks = [];
  const str = getFieldText(rawField).trim();
  if (!str) return { score, checks };
  const scoreMatch = str.match(/Quality\s*Score:\s*(\d+)/i);
  if (scoreMatch) score = scoreMatch[1];
  const verMatch = str.match(/Verified:\s*([\s\S]*)$/i);
  if (verMatch) checks = verMatch[1].split(",").map(s => s.trim()).filter(Boolean);
  return { score, checks };
}

function mapNebulaRecordToListItem(item) {
  const timing = parseTiming(item.totalCallTiming);
  const coursePitchedHuman = COURSE_REVERSE_MAP[item.coursePitched] || item.coursePitched || "";
  const leadStageHuman = LEAD_STAGE_REVERSE_MAP[item.leadStage] || item.leadStage || "";
  const callObjHuman = CALL_OBJECTIVE_REVERSE_MAP[item.callObjective] || item.callObjective || "";
  const callAttemptHuman = CALL_ATTEMPT_REVERSE_MAP[item.callAttempt] || item.callAttempt || "1st Attempt";
  const outcomeHuman = FINAL_OUTCOME_REVERSE_MAP[item.afterTheCallFinalCallOutcome] || item.afterTheCallFinalCallOutcome || "Submitted";
  const fitHuman = FIT_DECISION_REVERSE_MAP[item.duringTheCallFitDecision] || item.duringTheCallFitDecision || "";
  const nextActionHuman = NEXT_ACTION_REVERSE_MAP[item.afterTheCallNextActionRequired] || item.afterTheCallNextActionRequired || "";
  const validatedMotive = getFieldText(item.duringTheCallFinalValidatedMotive);

  const roleDim = parseTargetDimension(item.roleExperience, ["Level/Role", "Role/Experience", "Role"]);
  const skillDim = parseTargetDimension(item.skillToolingLevel, ["Current", "Skill/Tooling Level", "Skill"]);
  const compDim = parseTargetDimension(item.companyIndustry, ["Context", "Company/Industry", "Company"]);
  const painDim = parseTargetDimension(item.statedPainInterest, ["Pain", "Stated Pain/Interest", "Stated Pain"]);
  const urgDim = parseTargetDimension(item.timelineUrgency, ["Timeline", "Timeline/Urgency", "Timeline & Urgency"]);
  const parsedProbes = parseProbes(item.beforeTheCall5PreparedProbeQuestions);
  const parsedDiscovery = parseDiscovery(
    item.duringCallQuestionAsked,
    item.duringCallProspectSExactResponse,
    item.duringCallSignalsDetected,
    item.duringCallImmediateFollowUp
  );
  const parsedQs = parseQuestions(item.duringCallQuestionAsked);
  const parsedAns = parseResponses(item.duringCallProspectSExactResponse);
  const parsedFs = parseFollowUps(item.duringCallImmediateFollowUp);
  const parsedSigs = parseSignals(item.duringCallSignalsDetected);
  const parsedObjs = parseObjections(item.duringTheCallObjectionsLog);
  const parsedAct = parseActionDetails(item.afterTheCallActionOwnershipDueDate);
  const parsedQual = parseQualitySignOff(item.afterTheCallQualityReviewSignOff);

  const actionOwner = parsedAct.owner || "";
  const actionDueDate = parsedAct.due || "";
  const nextActions = (parsedAct.actions && parsedAct.actions.length > 0) ? parsedAct.actions : (nextActionHuman ? [nextActionHuman] : []);
  const objEvidenceNeeded = parsedAct.collateral || getFieldText(item.afterTheCallCollateralProofNeeded);

  const formData = {
    metaCaller: item.caller || "",
    metaDatetime: item.date ? (item.date.includes("T") ? item.date : item.date + "T10:00") : "",
    metaProspect: item.prospect || "",
    metaContact: item.contactInfo || "",
    metaSource: item.callSource || "",
    metaAttempt: callAttemptHuman,
    courses: coursePitchedHuman ? [coursePitchedHuman] : [],
    leadStages: leadStageHuman ? [leadStageHuman] : [],
    callObjectives: callObjHuman ? [callObjHuman] : [],
    targetRoleKnow: roleDim.know,
    targetRoleWhy: roleDim.why,
    targetSkillKnow: skillDim.know,
    targetSkillWhy: skillDim.why,
    targetCompanyKnow: compDim.know,
    targetCompanyWhy: compDim.why,
    targetPainKnow: painDim.know,
    targetPainWhy: painDim.why,
    targetUrgencyKnow: urgDim.know,
    targetUrgencyWhy: urgDim.why,
    hypoInitial: getFieldText(item.initialHypothesis),
    hypoEvidence: getFieldText(item.evidenceForHypothesis),
    hypoUnknown: getFieldText(item.whatIStillDoNotKnow),
    probes: parsedProbes,
    discovery: parsedDiscovery,
    discQ1: parsedQs.q1,
    discAns1: parsedAns.ans1,
    discF1: parsedFs.f1,
    sig1: parsedSigs.sig1,
    discQ2: parsedQs.q2,
    discAns2: parsedAns.ans2,
    discF2: parsedFs.f2,
    sig2: parsedSigs.sig2,
    discQ3: parsedQs.q3,
    discAns3: parsedAns.ans3,
    discF3: parsedFs.f3,
    synthInitial: getFieldText(item.duringTheCallWhatIThoughtBeforeCall),
    synthNew: getFieldText(item.duringTheCallWhatIDiscoveredDuringCall),
    synthFinal: validatedMotive,
    confidence: CONFIDENCE_REVERSE_MAP[item.duringTheCallConfidenceLevelInMotive] || "",
    fitDecision: fitHuman,
    fitEvidence: getFieldText(item.duringCallEvidenceForFit),
    pitch1: getFieldText(item.duringTheCallHook),
    pitch2: getFieldText(item.duringTheCallPainValidation),
    pitch3: getFieldText(item.riskOfInaction),
    pitch4: getFieldText(item.duringTheCallSolutionVision),
    pitch5: getFieldText(item.duringTheCallWhyThisTraining),
    pitch6: getFieldText(item.duringTheCallPracticalRelevance),
    pitch7: getFieldText(item.duringTheCallCallToAction),
    pitchProspectResponse: getFieldText(item.duringTheCallProspectResponseToPitch),
    closeAttempted: CLOSE_ATTEMPTED_REVERSE_MAP[item.duringTheCallCloseAttempted] || "",
    pitchDecision: DECISION_AT_PITCH_REVERSE_MAP[item.duringTheCallDecisionAtPitch] || "",
    objections: parsedObjs,
    objCategory: PRIMARY_OBJECTION_REVERSE_MAP[item.duringTheCallPrimaryObjectionCategory] || "",
    objPattern: "",
    objImprove: "",
    finalOutcome: outcomeHuman,
    nextActions: nextActions,
    actionOwner: actionOwner,
    actionDueDate: actionDueDate,
    objEvidenceNeeded: objEvidenceNeeded,
    learnings: [
      getFieldText(item.afterTheCallBuyingMotive),
      getFieldText(item.afterTheCallMostEffectiveProbe),
      getFieldText(item.afterTheCallWorstAssumptionIMade),
      getFieldText(item.afterTheCallObjectionIWasUnpreparedFor),
      getFieldText(item.afterTheCallScriptQuestionToImprove)
    ],
    mandatoryCheck: parsedQual.checks.length > 0 ? parsedQual.checks : ["target_hypo", "probes_5", "prospect_ans", "buying_motive", "custom_pitch", "obj_log", "outcome", "next_action"],
    sheetQuality: parsedQual.score || "5",
    readyNextCall: READY_NEXT_CALL_REVERSE_MAP[item.readyForNextCall] || "Yes",
    reviewerNotes: getFieldText(item.afterTheCallReviewerNotes),
    nebulaSyncStatus: "Synced to Nebula"
  };
  ...
  return {
    ...
    prospect: item.prospect || (item.name ? item.name.replace(/\s*-\s*\d{4}-\d{2}-\d{2}$/, "") : "Untitled Prospect"),
    caller: item.caller || "Sales Rep",
    date: item.date || (item.createdAt ? item.createdAt.slice(0, 10) : "N/A"),
    duration: timing.totalFormatted,
    stageTimes: { 1: timing.prepSec, 2: timing.liveSec, 3: timing.wrapSec },
    outcome: outcomeHuman,
    courses: coursePitchedHuman ? [coursePitchedHuman] : [],
    leadStages: leadStageHuman ? [leadStageHuman] : [],
    callObjectives: callObjHuman ? [callObjHuman] : [],
    contactInfo: item.contactInfo || "No contact info",
    callSource: item.callSource || "",
    callAttempt: callAttemptHuman,
    fitDecision: fitHuman,
    validatedMotive: validatedMotive,
    nextActions: nextActions,
    actionOwner: actionOwner,
    actionDueDate: actionDueDate,
    formData: formData,
...
function parseUrlRoute() {
  try {
    ...
    if (queryRoute) {
      if (queryRoute === "dashboard") path = "/dashboard";
      else if (queryRoute === "sheet") path = "/sheet";
    }

    if (window.location.hash) {
      const hash = window.location.hash.replace(/^#\/?/, "");
      if (hash.startsWith("dashboard")) path = "/dashboard";
      else if (hash.startsWith("sheet")) path = "/sheet";
      const qIdx = hash.indexOf("?");
      if (qIdx !== -1) search = hash.slice(qIdx);
    }

    const params = new URLSearchParams(search);
    const view = path.includes("sheet") ? "sheet" : "dashboard";
    const stage = parseInt(params.get("stage") || "1", 10);
    const id = params.get("id") || null;
    const mode = params.get("mode") || (id ? "view" : null);
    const page = parseInt(params.get("page") || "1", 10);

    return {
      view,
      stage: [1, 2, 3].includes(stage) ? stage : 1,
      id,
      mode,
      page: page > 0 ? page : 1
    };
  } catch {
    return { view: "dashboard", stage: 1, id: null, mode: null };
  }
}

function syncUrlToState(view, stage, recordId, viewMode, page = 1, replace = false) {
  try {
    let path = "/dashboard";
...
function LiveCallSheetApp() {
  ...
  const fetchNebulaRecords = async (showToast = true) => {
    ...
    setNebulaFetchError(null);
    try {
      const url = normalizeNebulaUrl(C);
      ...
      if (res.ok && data?.data?.liveCallSheets) {
        const liveSheets = data.data.liveCallSheets;
        const mappedList = liveSheets.map(mapNebulaRecordToListItem);

        setHistory(prev => {
          const nebulaIds = new Set(mappedList.map(m => m.id));
          const localOnly = (prev || []).filter(item => item && !nebulaIds.has(item.id) && item.source !== "nebula");
        ...
        setLastSyncedAt(now);
        if (showToast) {
          h("✨ Synced " + mappedList.length + " live records from Nebula CRM!");
        }
      } else {
        const msg = (data && (data.error || (Array.isArray(data.messages) ? data.messages.join("; ") : null))) || ("HTTP " + res.status);
        setNebulaFetchError(msg);
        if (showToast) h("⚠️ Nebula GET notice: " + msg);
      }
    } catch (err) {
      console.error("Nebula fetch error:", err);
      const msg = err.message || "Network request failed";
      setNebulaFetchError(msg);
      if (showToast) h("❌ Could not connect to Nebula: " + msg);
    } finally {
  ...
  const analytics = useMemo(() => {
    ...
    history.forEach(item => {
      ...
      const durSec = itemSec > 0 ? itemSec : parseTiming(item.duration || item.totalCallTiming).totalSec;
      totalSeconds += durSec;
    });
  ...
  const openRecordInViewMode = item => {
    let data = item.formData;
    if (item.rawNebula) {
      data = mapNebulaRecordToListItem(item.rawNebula).formData;
    } else if (!data) {
    ...
    setCurrentView("sheet");
    window.scrollTo({ top: 0, behavior: "smooth" });
    h(`🔒 Opened call record for "${item.prospect || "prospect"}" in View Mode (Read-Only)`);
  };

  const openRecordInEditMode = item => {
    if (confirm(`Open and edit call sheet for "${item.prospect || "prospect"}"? Current unsaved edits will be replaced.`)) {
      let data = item.formData;
      if (item.rawNebula) {
        data = mapNebulaRecordToListItem(item.rawNebula).formData;
      } else if (!data) {
      ...
      setCurrentView("sheet");
      window.scrollTo({ top: 0, behavior: "smooth" });
      h(`✏️ Edit Mode: Editing record for "${item.prospect || "prospect"}"`);
    }
  };

  const continueToFollowUp = item => {
    let baseData = item.formData;
    if (item.rawNebula) {
      baseData = mapNebulaRecordToListItem(item.rawNebula).formData;
    } else if (!baseData) {
      baseData = initialFormData;
    }
    const nextAttempt = (baseData.metaAttempt === "1st Attempt" || item.callAttempt === "1st Attempt" || item.callAttempt === "OPT1ST_ATTEMPT") ? "2nd Attempt" : "Follow-up";

    const now = new Date();
    const pad = n => String(n).padStart(2, "0");
    const currentDt = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`;

    p({
      ...baseData,
      metaAttempt: nextAttempt,
      metaDatetime: currentDt,
      finalOutcome: "",
      closeAttempted: "",
      pitchDecision: "",
      reviewerNotes: "",
      nebulaSyncStatus: "Not Synced"
    });
    ...
    setCurrentView("sheet");
    window.scrollTo({ top: 0, behavior: "smooth" });
    h(`📞 Follow-up Call Sheet ready for "${item.prospect || "prospect"}" (${nextAttempt})! Call timers reset.`);
  };

  const loadHistoryIntoSheet = openRecordInViewMode;
  useEffect(() => { try { localStorage.setItem(NEBULA_CONFIG_KEY, JSON.stringify({ url: C, token: w })) } catch { } }, [C, w]);
  useEffect(() => {
    syncUrlToState(currentView, l, b, isViewMode, currentPage, false);
  }, [currentView, l, b, isViewMode, currentPage]);
  ...
  useEffect(() => {
    if (initialRoute.id && history.length > 0) {
      const match = history.find(item => item.id === initialRoute.id);
      if (match) {
        if (initialRoute.mode === "view") openRecordInViewMode(match);
        else openRecordInEditMode(match);
      }
    }
  }, [history]); const N = useMemo(() => s[1] + s[2] + s[3], [s]), h = t => { $(t), setTimeout(() => { $("") }, 2800) }; useEffect(() => { try { const t = localStorage.getItem(STORAGE_KEY); t && p(JSON.parse(t)); const a = localStorage.getItem(TIMER_STORAGE_KEY); if (a) { const n = JSON.parse(a); n.stageTimes && S(n.stageTimes), n.stage && r(n.stage), n.isTimerRunning && u(!0) } } catch (t) { console.error("Error loading stored call sheet data", t) } }, []), useEffect(() => { localStorage.setItem(STORAGE_KEY, JSON.stringify(e)) }, [e]), useEffect(() => { let t = null; return d ? t = setInterval(() => { S(a => { const n = { ...a, [l]: a[l] + 1 }; return localStorage.setItem(TIMER_STORAGE_KEY, JSON.stringify({ stageTimes: n, stage: l, isTimerRunning: !0 })), n }) }, 1e3) : localStorage.setItem(TIMER_STORAGE_KEY, JSON.stringify({ stageTimes: s, stage: l, isTimerRunning: !1 })), () => clearInterval(t) }, [d, l]); const o = (t, a) => { if (isViewMode) return; if (!d && (u(!0), !e.metaDatetime)) { const n = new Date, i = g => String(g).padStart(2, "0"), c = `${n.getFullYear()}-${i(n.getMonth() + 1)}-${i(n.getDate())}T${i(n.getHours())}:${i(n.getMinutes())}`; p(g => ({ ...g, [t]: a, metaDatetime: c })); return } p(n => ({ ...n, [t]: a })) }, f = (t, a) => { if (isViewMode) return; d || u(!0), p(n => { const i = n[t] || []; return i.includes(a) ? { ...n, [t]: i.filter(c => c !== a) } : { ...n, [t]: [...i, a] } }) }, L = (t, a, n) => { if (isViewMode) return; d || u(!0), p(i => { const c = [...i.probes]; return c[t] = { ...c[t], [a]: n }, { ...i, probes: c } }) }, addProbeRow = () => { if (isViewMode) return; d || u(!0), p(prev => ({ ...prev, probes: [...(prev.probes || []), { q: "", m: "", f: "" }] })) }, deleteProbeRow = idx => { if (isViewMode) return; d || u(!0), p(prev => { const list = [...(prev.probes || [])]; if (list.length <= 1) return { ...prev, probes: [{ q: "", m: "", f: "" }] }; list.splice(idx, 1); return { ...prev, probes: list } }) }, E = (t, a, n) => { if (isViewMode) return; d || u(!0), p(i => { const c = [...i.objections]; return c[t] = { ...c[t], [a]: n }, { ...i, objections: c } }) }, addObjectionRow = () => { if (isViewMode) return; d || u(!0); p(prev => ({ ...prev, objections: [...(prev.objections || []), { exact: "", cause: "", resp: "", res: "Resolved" }] })); }, deleteObjectionRow = idx => { if (isViewMode) return; d || u(!0); p(prev => { const list = [...(prev.objections || [])]; if (list.length <= 1) return { ...prev, objections: [{ exact: "", cause: "", resp: "", res: "Resolved" }] }; list.splice(idx, 1); return { ...prev, objections: list }; }); }, handleDiscoveryChange = (idx, field, val) => {
    if (isViewMode) return;
    d || u(!0);
    p(prev => {
      const list = [...getDiscoveryList(prev)];
      list[idx] = { ...list[idx], [field]: val };
      const updates = { discovery: list };
      if (idx === 0) { if (field === 'q') updates.discQ1 = val; if (field === 'ans') updates.discAns1 = val; if (field === 'f') updates.discF1 = val; }
      else if (idx === 1) { if (field === 'q') updates.discQ2 = val; if (field === 'ans') updates.discAns2 = val; if (field === 'f') updates.discF2 = val; }
      else if (idx === 2) { if (field === 'q') updates.discQ3 = val; if (field === 'ans') updates.discAns3 = val; if (field === 'f') updates.discF3 = val; }
      return { ...prev, ...updates };
    });
  },
  handleDiscoverySignalToggle = (idx, sig) => {
    if (isViewMode) return;
    d || u(!0);
    p(prev => {
      const list = [...getDiscoveryList(prev)];
      const curr = list[idx]?.sigs || [];
      const nextSigs = curr.includes(sig) ? curr.filter(s => s !== sig) : [...curr, sig];
      list[idx] = { ...list[idx], sigs: nextSigs };
      const updates = { discovery: list };
      if (idx === 0) updates.sig1 = nextSigs;
      else if (idx === 1) updates.sig2 = nextSigs;
      return { ...prev, ...updates };
    });
  },
  addDiscoveryRow = () => {
...
`) || "No objections logged", ee = `Owner: ${e.actionOwner || "Unassigned"} | Due: ${e.actionDueDate || "N/A"}`, te = e.sheetQuality ? `Quality Rating: ${e.sheetQuality} / 5` : "", de = e.courses && e.courses.length > 0 && COURSE_PITCHED_MAP[e.courses[0]] || "PRO_CODE_AGENTIC_DEVELOPMENT", pe = e.leadStages && e.leadStages.length > 0 && LEAD_STAGE_MAP[e.leadStages[0]] || "FRESH_LEAD", he = e.callObjectives && e.callObjectives.length > 0 && CALL_OBJECTIVE_MAP[e.callObjectives[0]] || "QUALIFY", ue = e.nextActions && e.nextActions.length > 0 && NEXT_ACTION_MAP[e.nextActions[0]] || null, ae = { timingMetrics: { stage1_beforeCall_seconds: s[1], stage1_formatted: formatDuration(s[1]), stage2_duringCall_seconds: s[2], stage2_formatted: formatDuration(s[2]), stage3_afterCall_seconds: s[3], stage3_formatted: formatDuration(s[3]), totalCall_seconds: N, totalCall_formatted: formatDuration(N) }, clientApp: "Nebula CRM Live Call Sheet", version: "2.5", allSelectedCourses: e.courses, allSelectedLeadStages: e.leadStages, allSelectedObjectives: e.callObjectives, allSelectedNextActions: e.nextActions }; return { name: e.metaProspect ? `Live Call - ${e.metaProspect}` : "Live Sales Call Record", id: generateUUID(), createdAt: t, createdBySource: "APPLICATION", createdByWorkspaceMemberId: null, createdByName: e.metaCaller || "Sales Representative", createdByContext: ae, deletedAt: null, position: 1, searchVector: null, updatedAt: t, updatedBySource: "APPLICATION", updatedByWorkspaceMemberId: null, updatedByName: e.metaCaller || "Sales Representative", updatedByContext: ae, caller: e.metaCaller || null, date: e.metaDatetime ? e.metaDatetime.slice(0, 10) : t.slice(0, 10), prospect: e.metaProspect || null, contactInfo: e.metaContact || null, callSource: e.metaSource || null, callAttempt: CALL_ATTEMPT_MAP[e.metaAttempt] || "OPT1ST_ATTEMPT", coursePitched: de, leadStage: pe, callObjective: he, roleExperienceBlocknote: toBlocknote(a), roleExperienceMarkdown: a, skillToolingLevelBlocknote: toBlocknote(n), skillToolingLevelMarkdown: n, companyIndustryBlocknote: toBlocknote(i), companyIndustryMarkdown: i, statedPainInterestBlocknote: toBlocknote(c), statedPainInterestMarkdown: c, timelineUrgencyBlocknote: toBlocknote(g), timelineUrgencyMarkdown: g, initialHypothesisBlocknote: toBlocknote(e.hypoInitial), initialHypothesisMarkdown: toMarkdown(e.hypoInitial), evidenceForHypothesisBlocknote: toBlocknote(e.hypoEvidence), evidenceForHypothesisMarkdown: toMarkdown(e.hypoEvidence), whatIStillDoNotKnowBlocknote: toBlocknote(e.hypoUnknown), whatIStillDoNotKnowMarkdown: toMarkdown(e.hypoUnknown), beforeTheCall5PreparedProbeQuestionsBlocknote: toBlocknote(I), beforeTheCall5PreparedProbeQuestionsMarkdown: I, duringCallQuestionAskedBlocknote: toBlocknote(J), duringCallQuestionAskedMarkdown: J, duringCallProspectSExactResponseBlocknote: toBlocknote(q), duringCallProspectSExactResponseMarkdown: q, duringCallSignalsDetectedBlocknote: toBlocknote(z), duringCallSignalsDetectedMarkdown: z, duringCallImmediateFollowUpBlocknote: toBlocknote(X), duringCallImmediateFollowUpMarkdown: X, duringTheCallWhatIThoughtBeforeCallBlocknote: toBlocknote(e.synthInitial), duringTheCallWhatIThoughtBeforeCallMarkdown: toMarkdown(e.synthInitial), duringTheCallWhatIDiscoveredDuringCallBlocknote: toBlocknote(e.synthNew), duringTheCallWhatIDiscoveredDuringCallMarkdown: toMarkdown(e.synthNew), duringTheCallFinalValidatedMotiveBlocknote: toBlocknote(e.synthFinal), duringTheCallFinalValidatedMotiveMarkdown: toMarkdown(e.synthFinal), duringTheCallConfidenceLevelInMotive: CONFIDENCE_MAP[e.confidence] || null, duringTheCallFitDecision: FIT_DECISION_MAP[e.fitDecision] || null, duringCallEvidenceForFitBlocknote: toBlocknote(e.fitEvidence), duringCallEvidenceForFitMarkdown: toMarkdown(e.fitEvidence), duringTheCallHookBlocknote: toBlocknote(e.pitch1), duringTheCallHookMarkdown: toMarkdown(e.pitch1), duringTheCallPainValidationBlocknote: toBlocknote(e.pitch2), duringTheCallPainValidationMarkdown: toMarkdown(e.pitch2), riskOfInactionBlocknote: toBlocknote(e.pitch3), riskOfInactionMarkdown: toMarkdown(e.pitch3), duringTheCallSolutionVisionBlocknote: toBlocknote(e.pitch4), duringTheCallSolutionVisionMarkdown: toMarkdown(e.pitch4), duringTheCallWhyThisTrainingBlocknote: toBlocknote(e.pitch5), duringTheCallWhyThisTrainingMarkdown: toMarkdown(e.pitch5), duringTheCallPracticalRelevanceBlocknote: toBlocknote(e.pitch6), duringTheCallPracticalRelevanceMarkdown: toMarkdown(e.pitch6), duringTheCallCallToActionBlocknote: toBlocknote(e.pitch7), duringTheCallCallToActionMarkdown: toMarkdown(e.pitch7), duringTheCallProspectResponseToPitchBlocknote: toBlocknote(e.pitchProspectResponse), duringTheCallProspectResponseToPitchMarkdown: toMarkdown(e.pitchProspectResponse), duringTheCallCloseAttempted: CLOSE_ATTEMPTED_MAP[e.closeAttempted] || null, duringTheCallDecisionAtPitch: DECISION_AT_PITCH_MAP[e.pitchDecision] || null, duringTheCallObjectionsLogBlocknote: toBlocknote(Z), duringTheCallObjectionsLogMarkdown: Z, duringTheCallPrimaryObjectionCategory: PRIMARY_OBJECTION_MAP[e.objCategory] || null, afterTheCallFinalCallOutcome: FINAL_OUTCOME_MAP[e.finalOutcome] || null, afterTheCallNextActionRequired: ue, afterTheCallActionOwnershipDueDateBlocknote: toBlocknote(ee), afterTheCallActionOwnershipDueDateMarkdown: ee, afterTheCallCollateralProofNeededBlocknote: toBlocknote(e.objEvidenceNeeded), afterTheCallCollateralProofNeededMarkdown: toMarkdown(e.objEvidenceNeeded), afterTheCallBuyingMotiveBlocknote: toBlocknote(e.learnings[0]), afterTheCallBuyingMotiveMarkdown: toMarkdown(e.learnings[0]), afterTheCallMostEffectiveProbeBlocknote: toBlocknote(e.learnings[1]), afterTheCallMostEffectiveProbeMarkdown: toMarkdown(e.learnings[1]), afterTheCallWorstAssumptionIMadeBlocknote: toBlocknote(e.learnings[2]), afterTheCallWorstAssumptionIMadeMarkdown: toMarkdown(e.learnings[2]), afterTheCallObjectionIWasUnpreparedForBlocknote: toBlocknote(e.learnings[3]), afterTheCallObjectionIWasUnpreparedForMarkdown: toMarkdown(e.learnings[3]), afterTheCallScriptQuestionToImproveBlocknote: toBlocknote(e.learnings[4]), afterTheCallScriptQuestionToImproveMarkdown: toMarkdown(e.learnings[4]), afterTheCallQualityReviewSignOffBlocknote: toBlocknote(te), afterTheCallQualityReviewSignOffMarkdown: te, readyForNextCall: READY_NEXT_CALL_MAP[e.readyNextCall] || null, afterTheCallReviewerNotesBlocknote: toBlocknote(e.reviewerNotes), afterTheCallReviewerNotesMarkdown: toMarkdown(e.reviewerNotes) }
    ...
    const curDisc = getDiscoveryList(e);
    const discQMd = curDisc.map((d, i) => d.q ? `Q${i + 1}: ${d.q}` : "").filter(Boolean).join("\n");
    const discAnsMd = curDisc.map((d, i) => d.ans ? `Ans ${i + 1}: ${d.ans}` : "").filter(Boolean).join("\n");
    const sigsMd = curDisc.map((d, i) => (d.sigs && d.sigs.length) ? `Q${i + 1} Signals: ${d.sigs.join(", ")}` : "").filter(Boolean).join(" | ");
    const discFMd = curDisc.map((d, i) => d.f ? `Follow-up ${i + 1}: ${d.f}` : "").filter(Boolean).join("\n");
    ...
    try {
      ...
      if (isUpdate) {
        usedMethod = "PATCH";
        requestUrl = `${t}/${editingRecordId}`;
        n = await fetch(requestUrl, {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${w.trim()}`,
            "Content-Type": "application/json"
        ...
        // If PATCH returns 404 (e.g. record only existed locally or in mock), fallback to POST
        if (n.status === 404) {
          usedMethod = "POST";
          requestUrl = t;
          n = await fetch(requestUrl, {
      ...
      if (n.status === 200 || n.status === 201 || (n.ok && i?.data)) {
        const remoteId = (i?.data?.createLiveCallSheet || i?.data?.updateLiveCallSheet || i?.data)?.id;
        const finalId = (isUpdate && usedMethod === "PATCH") ? editingRecordId : (remoteId || editingRecordId || "ID-" + Math.floor(1e5 + Math.random() * 9e5));
        ...
        setHistory(prev => {
          let up;
          if (isUpdate) {
            const idx = prev.findIndex(item => item.id === editingRecordId);
            if (idx !== -1) {
              up = [...prev];
              up[idx] = {
                ...prev[idx],
                ...savedItem,
                id: finalId,
                timestamp: prev[idx].timestamp || savedItem.timestamp,
        ...
        window.scrollTo({ top: 0, behavior: "smooth" });
        h(isUpdate ? `✅ Call record for "${e.metaProspect || "Prospect"}" updated in Nebula CRM & Records!` : `🚀 Call submitted to Nebula CRM (ID: ${finalId}) & logged to Records!`);
        fetchNebulaRecords(false);
      } else {
...
==============================
    ...
    React.createElement("main", { className: "document-container" },
      currentView === "dashboard" ? (
        React.createElement("div", { className: "dashboard-container fade-in" },
          ...
          filteredHistory.length === 0 ? (
                ...
                history.length === 0
                  ? React.createElement(React.Fragment, null,
                    React.createElement("button", {
                      ...
                      onClick: () => fetchNebulaRecords(true),
                      disabled: isLoadingNebula
                    }, isLoadingNebula ? "🔄 Fetching..." : "🔄 Fetch from Nebula CRM"),
                    React.createElement("button", {
            ...
            React.createElement("div", { className: "table-responsive-wrap" },
                      ...
                      React.createElement("td", null,
                        React.createElement("div", { className: "record-prospect-cell" },
                          ...
                          React.createElement("div", { className: "record-prospect-info" },
                            ...
                            (item.source === "nebula" || item.rawNebula) && React.createElement("span", { className: "nebula-source-pill", title: `Nebula Record ID: ${item.id}` }, "🌌 Nebula CRM"),
              ...
              React.createElement("div", { className: "table-pagination-bar no-print" },
                React.createElement("div", { className: "pagination-info" },
                  React.createElement("span", null,
                    `Showing ${filteredHistory.length === 0 ? 0 : (validCurrentPage - 1) * pageSize + 1}–${Math.min(validCurrentPage * pageSize, filteredHistory.length)} of ${filteredHistory.length} record${filteredHistory.length === 1 ? "" : "s"}`
                  ),
                  React.createElement("div", { className: "page-size-selector" },
                    React.createElement("label", { style: { fontSize: "12px", color: "#64748b" } }, "Per page:"),
                    React.createElement("select", {
                      value: pageSize,
                      onChange: (e) => {
                ...
                totalPages > 1 && React.createElement("div", { className: "pagination-controls" },
                  ...
                  Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((pageItem, pIdx) =>
                      pageItem === "..." ? (
                        React.createElement("span", { key: `ellipsis-${pIdx}`, className: "pagination-ellipsis" }, "...")
                      ) : (
                        React.createElement("button", {
                          key: pageItem,
                          type: "button",
                          className: `pagination-page-btn ${validCurrentPage === pageItem ? "active" : ""}`,
        ...
        React.createElement(React.Fragment, null,
          isViewMode ? (
            React.createElement("div", { className: "view-mode-banner no-print" },
              React.createElement("div", { className: "view-mode-info" },
                React.createElement("span", { className: "view-mode-badge" }, "🔒 VIEW MODE (READ-ONLY)"),
                React.createElement("span", { className: "view-mode-title" },
                  `Viewing Call Record: ${e.metaProspect || "Untitled Prospect"} · ${e.metaCaller || "Sales Rep"} (${formatDisplayDateTime(e.metaDatetime)})`
                )
              ),
              React.createElement("div", { className: "view-mode-actions" },
                React.createElement("button", {
                  type: "button",
                  className: "btn-edit-mode",
                  onClick: () => {
                    setIsViewMode(false);
                    setEditingRecordId(b);
                    h("✏️ Edit Mode Enabled! Modifying existing record.");
                  }
                }, "✏️ Enable Edit Mode"),
                React.createElement("button", {
                  type: "button",
            ...
            React.createElement("div", { className: "edit-mode-active-banner no-print" },
              React.createElement("div", { className: "edit-mode-info" },
                React.createElement("span", { className: "edit-mode-badge" }, "✏️ EDIT MODE ACTIVE"),
                React.createElement("span", null, `Editing Record ID: ${b} · Changes can be updated and submitted to Nebula CRM.`)
              ),
              React.createElement("div", { style: { display: "flex", gap: "8px" } },
                React.createElement("button", {
                  ...
                  onClick: () => {
                    setIsViewMode(true);
                    h("🔒 Switched to View Mode (Read-Only)");
                  }
                }, "🔒 Switch to View Mode"),
                React.createElement("button", {
                  type: "button",
    ...
    getDiscoveryList(e).map((t, a) =>
      React.createElement("tr", { key: a },
        ...
        React.createElement("td", null,
          React.createElement("textarea", {
            value: t.f || "",
            onChange: n => handleDiscoveryChange(a, "f", n.target.value),
            placeholder: "Immediate follow-up asked..."
          })
        ),
        !isViewMode && React.createElement("td", { style: { textAlign: "center", verticalAlign: "middle" } },
          React.createElement("button", {
            type: "button",
            className: "probe-row-del-btn",
            onClick: () => deleteDiscoveryRow(a),
            title: `Delete discovery question #${a + 1}`,
            disabled: getDiscoveryList(e).length <= 1
          }, "🗑️")
        )
      )
    )
