...
function mapNebulaRecordToListItem(item) {
  ...
  const actionDueDateText = getFieldText(item.afterTheCallActionOwnershipDueDate);
  let actionOwner = "";
  let actionDueDate = "";
  if (actionDueDateText) {
    const ownerMatch = actionDueDateText.match(/Owner:\s*([^|]+)/i);
    if (ownerMatch) actionOwner = ownerMatch[1].trim();
    const dueMatch = actionDueDateText.match(/Due:\s*([^|]+)/i);
    if (dueMatch) actionDueDate = dueMatch[1].trim();
  }
  ...
  const formData = {
    ...
    targetRoleKnow: getFieldText(item.roleExperience),
    targetRoleWhy: "",
    targetSkillKnow: getFieldText(item.skillToolingLevel),
    targetSkillWhy: "",
    targetCompanyKnow: getFieldText(item.companyIndustry),
    targetCompanyWhy: "",
    targetPainKnow: getFieldText(item.statedPainInterest),
    targetPainWhy: "",
    targetUrgencyKnow: getFieldText(item.timelineUrgency),
    targetUrgencyWhy: "",
    ...
    probes: [{ q: getFieldText(item.beforeTheCall5PreparedProbeQuestions), m: "", f: "" }, { q: "", m: "", f: "" }, { q: "", m: "", f: "" }, { q: "", m: "", f: "" }, { q: "", m: "", f: "" }],
    discQ1: getFieldText(item.duringCallQuestionAsked),
    discAns1: getFieldText(item.duringCallProspectSExactResponse),
    discF1: getFieldText(item.duringCallImmediateFollowUp),
    sig1: [],
    discQ2: "",
    discAns2: "",
    discF2: "",
    ...
    objections: [{ exact: getFieldText(item.duringTheCallObjectionsLog), cause: "", resp: "", res: "Resolved" }, { exact: "", cause: "", resp: "", res: "" }],
    objCategory: PRIMARY_OBJECTION_REVERSE_MAP[item.duringTheCallPrimaryObjectionCategory] || "",
    objPattern: "",
    objImprove: "",
    finalOutcome: outcomeHuman,
    nextActions: nextActionHuman ? [nextActionHuman] : [],
    actionOwner: actionOwner,
    actionDueDate: actionDueDate,
    objEvidenceNeeded: getFieldText(item.afterTheCallCollateralProofNeeded),
    learnings: [
    ...
    mandatoryCheck: ["target_hypo", "probes_5", "prospect_ans", "buying_motive", "custom_pitch", "obj_log", "outcome", "next_action"],
    sheetQuality: "5",
...
const openRecordInViewMode = item => {
  const data = item.formData || (item.rawNebula ? mapNebulaRecordToListItem(item.rawNebula).formData : initialFormData);
  p(data);
...
const continueToFollowUp = item => {
  const baseData = item.formData || (item.rawNebula ? mapNebulaRecordToListItem(item.rawNebula).formData : initialFormData);
